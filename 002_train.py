...
if __name__ == '__main__':
    #多卡并行训练指令：python -m torch.distributed.run --nproc_per_node 6 /root/001_Code/Su77_YOLOv11_Improve/002_train_Ours.py --batch 320（4是指卡的数量）
    # 加载模型
    model = YOLO(r'/root/001_Code/DQCF-Net/ultralytics/cfg/models/DQCF-Net-T.yaml')  # 不使用预训练权重训练
    # model = YOLO(r'yolov8.yaml').load("yolov8n.pt")  # 使用预训练权重训练
    # 训练参数 ----------------------------------------------------------------------------------------------
    model.train(
        data=r'/root/002_Dataset/Path/My_YoloData_UAV.yaml',
        resume=False,  # (bool) 从上一个检查点恢复训练
        epochs=200,  # (int) 训练的周期数
        patience=50,  # (int) 等待无明显改善以进行早期停止的周期数
        batch=320,  # (int) 每批次的图像数量（-1 为自动批处理）
        imgsz=640,  # (int) 输入图像的大小，整数或w，h
        save=True,  # (bool) 保存训练检查点和预测结果
        save_period=-1,  # (int) 每x周期保存检查点（如果小于1则禁用）
        cache=False,  # (bool) True/ram、磁盘或False。使用缓存加载数据
        device='0,1,2,3,4,5',  # (int | str | list, optional) 运行的设备，例如 cuda device=0 或 device=0,1,2,3 或 device=cpu
        workers=8,  # (int) 数据加载的工作线程数（每个DDP进程）
        project='/root/003_Experiment',  # (str, optional) 项目名称
        name='IDD-YOLOv11_0320',  # (str, optional) 实验名称，结果保存在'project/name'目录下
